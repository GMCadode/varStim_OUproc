...
function [stim] = genRegularStimtrain(totaltime, srate, avISIms, percStdms)
...
Fs_DBS = 1/(avISIms/1e3); % Hz, pulses per sec
mu_ISI = 1/Fs_DBS;
num_pulse = floor(T*Fs_DBS);
ISI = mu_ISI * ones(num_pulse,1);

stim = zeros(floor(T*srate),1); 
stim(floor(cumsum(ISI*srate))) = 1; % assign 1's for the stim time indices
